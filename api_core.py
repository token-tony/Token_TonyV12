...
async def fetch_ipfs_json(uri: str) -> Optional[Dict[str, Any]]:
    """Resolve an IPFS URI using a set of HTTP gateways."""
    if not _is_ipfs_uri(uri):
        result = await fetch(uri, provider="ipfs")
    ...
    for idx, url in enumerate(gateways):
        result = await fetch(url, timeout=timeout_s, provider="ipfs")
...
async def fetch_helius_asset(mint: str) -> Optional[Dict[str, Any]]:
    ...
    result = await fetch(HELIUS_RPC_URL, method="POST", json_data=payload, provider="helius")
    return result if isinstance(result, dict) else None


async def fetch_top10_via_rpc(mint: str) -> Optional[Dict[str, Any]]:
    ...
    accounts = await fetch(HELIUS_RPC_URL, method="POST", json_data=payload_accounts, provider="helius")
    supply = await fetch(HELIUS_RPC_URL, method="POST", json_data=payload_supply, provider="helius")
...
async def fetch_holders_count_via_rpc(mint: str) -> Optional[int]:
    data = await fetch_top10_via_rpc(mint)
    if data:
        return int(data.get("holders_count") or 0)
    return None


async def fetch_birdeye(mint: str) -> Optional[Dict[str, Any]]:
    if not BIRDEYE_API_KEY:
        return None
    url = f"https://public-api.birdeye.so/public/marketstat/solana/{mint}"
    headers = {"X-API-KEY": BIRDEYE_API_KEY, "Accept": "application/json"}
    result = await fetch(url, headers=headers, provider="birdeye")
    return result if isinstance(result, dict) else None


async def fetch_dexscreener_by_mint(identifier: str) -> Optional[Dict[str, Any]]:
    """Fetch token details from DexScreener by mint address."""
    url = f"https://api.dexscreener.com/latest/dex/tokens/{identifier}"
    result = await fetch(url, provider="dexscreener", ds=True)
...
async def _fetch_dexscreener_pair(pair_address: str) -> Optional[Dict[str, Any]]:
...
async def fetch_gecko_market_data(mint: str) -> Optional[Dict[str, Any]]:
    headers = {"Accept": "application/json;version=20230302"}
    url = f"{GECKO_API_URL}/networks/solana/tokens/{mint}?include=market_data"  # type: ignore[str-format]
    result = await fetch(url, headers=headers, provider="gecko")
    if not isinstance(result, dict):
        return None
...
async def fetch_market_snapshot(mint: str) -> Optional[Dict[str, Any]]:
    # Primary: DexScreener
    ds = await fetch_dexscreener_by_mint(mint)
    if ds:
        return ds
    # Secondary: BirdEye
    be = await fetch_birdeye(mint)
    ...
    return await fetch_gecko_market_data(mint)


async def fetch_jupiter_has_route(mint: str) -> Optional[bool]:
    params = {
        "inputMint": mint,
    ...
    result = await fetch(JUP_QUOTE_URL, params=params, provider="jupiter")
    if not isinstance(result, dict):
        return None
...
async def fetch_rugcheck_score(mint: str) -> Optional[str]:
    url = f"{RUGCHECK_API_URL.rstrip('/')}/token/{mint}"
    headers = {"Accept": "application/json"}
    if RUGCHECK_JWT:
        headers["Authorization"] = f"Bearer {RUGCHECK_JWT}"
    result = await fetch(url, headers=headers, provider="rugcheck")
    if not isinstance(result, dict):
        return None
...
async def fetch_creator_dossier_bitquery(creator: str) -> Optional[int]:
...
async def fetch_twitter_stats(url_or_handle: str) -> Optional[Dict[str, Any]]:
    ...
    user = await fetch(url, params=params, headers=headers, provider="twitter")
...
async def extract_mint_from_check_text(text: str) -> Optional[str]:
    if not text:
        return None
    cleaned = text.strip()
    ...
    if pair_match:
        ...
        ds_pair = await _fetch_dexscreener_pair(pair)
        if ds_pair:
...
async def fetch_holders_via_program_accounts(mint: str) -> Optional[int]:
...
async def fetch_market_pair_address(mint: str) -> Optional[str]:
...
from utils import is_valid_solana_address, HTTP_LIMITER
